                ...
                <span class="wpcf7-form-control-wrap menu-cities">
                    <select name="city_id" class="wpcf7-form-control wpcf7-select wpcf7-validates-as-required"
                       ...
                       @foreach($cities as $city)
                            <option
                                value="{{ $city->id }}" {{ old('city_id') == $city->id ? 'selected' : null }}>{{ $city->name }}</option>
                        @endforeach
                    </select>
                </span>
        </p>
    ...
    <div class="column one">
        ...
        <p>
            <label>
                ...
                <input type="text" name="school" value="{{ old('school') }}" size="40"
            ...
            <span class="wpcf7-form-control-wrap menu-english_students">
                <select name="class_id" class="wpcf7-form-control wpcf7-select" aria-invalid="false" required>
                    ...
                    @foreach($schoolClass as $class)
                        <option
                            value="{{ $class->id }}" {{ old('class_id') == $class->id ? 'selected' : null }}>{{ $class->class_name }}</option>
                    @endforeach
                </select>
            </span>
        </p>
        ...
        <p>
            <label>
                <span class="wpcf7-form-control-wrap students3">
                    <input type="text" name="favorite_subjects" value="{{ old('favorite_subjects') }}"
                           size="40"
                           class="wpcf7-form-control wpcf7-text wpcf7-validates-as-required"
                           aria-required="true" aria-invalid="false" required>
                </span>
                ...
                <span class="wpcf7-form-control-wrap menu-english_students">
                    <select name="english_level_id"
                        ...
                        @foreach($englishLevels as $englishLevel)
                            <option
                                value="{{ $englishLevel->id }}" {{ old('english_level_id') == $englishLevel->id ? 'selected' : null }}>{{ $englishLevel->level }}</option>
                        @endforeach
                    </select>
                </span>
        </p>
                ...
                <span class="wpcf7-form-control-wrap menu-english_students">
                    <select name="sport_id" class="wpcf7-form-control wpcf7-select" aria-invalid="false" required>
                        ...
                        @foreach($sports as $sport)
                            <option
                                value="{{ $sport->id }}" {{ old('sport_id') == $sport->id ? 'selected' : null }}>{{ $sport->name }}</option>
                        @endforeach
                    </select>
                 </span>
        </p>
                ...
                <span class="wpcf7-form-control-wrap menu-cities">
                    <select name="spheres[]" class="wpcf7-form-control wpcf7-select select2 wpcf7-validates-as-required"
                       ...
                       @foreach($spheres as $sphere)
                            ...
                            {{ old('spheres') && in_array($sphere->id, old('spheres')) ? 'selected' : null }}>{{ $sphere->name }}</option>
                        @endforeach
                    </select>
                </span>
        </p>
        ...
        <p>
                <span class="wpcf7-form-control-wrap menu-time">
                    <select name="hours"
                            ...
                            aria-invalid="false" required>
                        <option value="">---</option>
                        <option value="1" {{ old('hours') == '1' ? 'selected' : null }}>1 час</option>
                        <option value="2" {{ old('hours') == '2' ? 'selected' : null }}>2 часа</option>
                        <option value="3" {{ old('hours') == '3' ? 'selected' : null }}>3 часа</option>
                        <option value="4" {{ old('hours') == '4' ? 'selected' : null }}>4 часа</option>
                        <option value="5" {{ old('hours') == '5' ? 'selected' : null }}>повече</option>
                    </select>
                 </span>
        </p>
    </div>
        ...
        <p>
            <label style="margin-bottom:50px;">
                    <span class="wpcf7-form-control-wrap message">
                        <input type="text" name="able_mentor_info_source"
                               value="{{ old('able_mentor_info_source') }}" size="40"
                               class="wpcf7-form-control wpcf7-text wpcf7-validates-as-required" aria-required="true"
                               aria-invalid="false" required>
                    </span>
            </label>
